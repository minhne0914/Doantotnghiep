...
# Module-level constant để các app khác (vd: home, emr) có thể import
DEPARTMENT_CHOICES = (
...
# Backward-compat alias (code cũ tham chiếu `department` lower-case)
...
class Appointment(models.Model):
    ...
    department = models.CharField(choices=DEPARTMENT_CHOICES, max_length=100, db_index=True)
...
class TakeAppointment(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(fields=['user', 'date']),
            models.Index(fields=['appointment', 'date']),
...
class AppointmentChangeLog(models.Model):
    ...
    booking = models.ForeignKey(
        TakeAppointment, on_delete=models.CASCADE, related_name='change_logs'
    )
...
class DoctorReview(models.Model):
    ...
    booking = models.OneToOneField(
        TakeAppointment, on_delete=models.CASCADE, related_name='review'
    )
    ...
    def __str__(self):
        return (
            f'Review for Dr. {self.doctor.last_name} '
            f'by {self.patient.last_name} - {self.rating} Stars'
        )


class DirectMessage(models.Model):
    booking = models.ForeignKey(
        TakeAppointment, on_delete=models.CASCADE, related_name='messages'
    )
